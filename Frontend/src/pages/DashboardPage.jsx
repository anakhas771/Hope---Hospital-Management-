...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      try {
        const userData = await apiFetch("/user/profile/");      // ✅ Correct profile endpoint
        const apptData = await apiFetch("/appointments/");      // ✅ Correct appointments endpoint

        setUser(userData);
        setAppointments(apptData.sort((a, b) => new Date(b.date_time) - new Date(a.date_time)));
      } catch (e) {
  ...
  const cancelAppointment = async (id) => {
    ...
    setAppointments(updated); // Optimistic UI update
    ...
    toast((t) => (
      <div className="text-center">
        ...
        <button
          ...
          onClick={() => {
            setAppointments(prev => [...prev, deleted].sort((a,b)=>new Date(b.date_time)-new Date(a.date_time)));
            toast.dismiss(t.id);
          }}
        >
          Undo
    ...
    try {
      await apiFetch(`/appointments/${id}/cancel/`, "POST"); // backend cancel
    } catch (e) {
  ...
  return (
    <div className="bg-white/50 backdrop-blur-xl shadow-xl rounded-3xl p-6">
      <div className="flex flex-col items-center">
        <img src={user.profile_image || "/default-avatar.png"} className="w-28 h-28 rounded-full border-4 border-white shadow-xl object-cover" alt="Profile" />
...
function AppointmentsList({ appointments, cancelAppointment }) {
  return (
    <div className="bg-white/60 backdrop-blur-xl p-6 rounded-3xl shadow-lg">
      ...
      {appointments.length === 0 && <p className="text-center text-gray-600 py-6">No appointments yet.</p>}
      <div className="space-y-4">
        {appointments.map(item => (
          <div key={item.id} className="flex justify-between items-center bg-white/70 p-4 rounded-xl shadow hover:scale-[1.01] transition-all">
            <p className="font-bold text-gray-700">{new Date(item.date_time).toDateString()}</p>
            <div className="w-1/3">
              <p className="font-semibold">{item.doctor?.name || "Unknown Doctor"}</p>
              <p className="text-xs text-gray-500">{item.notes || "No notes"}</p>
              {item.doctor?.specialization && <p className="text-xs text-gray-400">{item.doctor.specialization}</p>}
            </div>
            <span className={`px-3 py-1 rounded-full text-sm ${item.status==="Done"?"bg-green-100 text-green-600":item.status==="Cancelled"?"bg-red-100 text-red-600":"bg-blue-100 text-blue-600"}`}>{item.status}</span>
            <p className="font-semibold">₹{item.amount}</p>
            <div className="flex gap-2">
              <button onClick={()=>cancelAppointment(item.id)} disabled={item.status==="Cancelled"} className={`px-3 py-1 rounded-xl text-sm ${item.status==="Cancelled"?"bg-gray-400 cursor-not-allowed":"bg-red-500 hover:bg-red-600 text-white"}`}>Cancel</button>
